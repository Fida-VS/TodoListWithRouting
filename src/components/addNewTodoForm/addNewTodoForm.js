...
export const AddNewTodoForm = ({ value, setValue, requestAddNewTodo, getSortedTodos, onSearch }) => {
	...
	return (
			...
			<form className={styles.addNewTodoForm}>
				...
				<button onClick={() => getSortedTodos()} className={styles.buttonAlphabeticalSorting}>По алфавиту</button>
				<button onClick={() => onSearch()} className={styles.buttonSearch}>🔎</button>
			</form>
